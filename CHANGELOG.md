...
- `run_shell` 升级 1.2.0：Shell 命令工作目录改为可配置（默认 `data/run_shell`，`{REPO_ROOT}` 模板展开 + 启动自动创建，WebUI「配置」可改），执行命令不再落在 `installed/` 程序目录内——更新/重装/卸载都不会清掉工作目录内容；临时输出图片也随工作目录走，字体仍由程序包自带（不受影响）
