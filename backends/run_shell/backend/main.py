...
TOKEN = os.environ.get("ERROR_BACKEND_TOKEN") or "123456"  # 优先 error-backends 注入的 token
...
def authorized(request: Request, token: str = "") -> bool:
    """兼容 query 参数 token 与 error-backends 约定的请求头（Authorization: Bearer / X-Token）"""
    if token == TOKEN:
        return True
...
async def create_process(request: Request, token: str = Query(""), cmd: str = Query(..., description="Shell命令字符串")):
    """创建新进程并返回UUID"""
    if not authorized(request, token):
        raise HTTPException(status_code=401, detail="无效的token")
...
@app.get("/del_process")
async def del_process(request: Request, token: str = Query(""), pid: str = Query(..., description="进程UUID")):
    """删除指定进程"""
    if not authorized(request, token):
        raise HTTPException(status_code=401, detail="无效的token")
...
async def list_process(request: Request, token: str = Query("")):
    """列出所有进程"""
    if not authorized(request, token):
        raise HTTPException(status_code=401, detail="无效的token")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        app,
        host=os.environ.get("ERROR_BACKEND_HOST", "127.0.0.1"),
        port=int(os.environ.get("ERROR_BACKEND_PORT", "3011")),
    )
