...
每个后端卡片在「依赖已装且运行中」时显示「重启」按钮，可单独重启该后端（等价 `errorbackend restart <后端名>`）。

每个后端卡片展示版本号（`backend.json` 的 `version` 字段）；后台按 60 秒间隔 `git fetch` 对比远端版本，有新版时卡片出现「⬆ 更新」按钮、右上角更新按钮出现小红点，点击可拉取代码并检查依赖、重启该后端。「删除」按钮会连同依赖一起删除整个后端目录（删除已 git 暂存，推送远端后永久生效）。
