...
一条命令搞定全部：首次运行自动安装所需依赖，并在**后台启动**管理界面（不占用终端、无控制台窗口），随后自动打开 http://127.0.0.1:8911（仅本机）。停止后台 WebUI：`python launcher.py webui-stop` 或 `errorbackend webui-stop`。所有管理都在页面里完成：
